...
contract ZonuDexFeeReceiver {
    using SafeMath for uint;

    address public owner;
    IZonuDexFactory public factory;
    address public WETH;
    ...
    constructor(
        ...
        owner = _owner;
        factory = IZonuDexFactory(_factory);
        WETH = _WETH;
    ...
    function transferOwnership(address newOwner) external {
        require(msg.sender == owner, 'ZonuDexFeeReceiver: FORBIDDEN');
        owner = newOwner;
    }
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'ZonuDexFeeReceiver: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'ZonuDexFeeReceiver: ZERO_ADDRESS');
    }
    ...
    // Taken from ZonuDexLibrary, removed the factory parameter
    ...
    // Done with code form ZonuDexRouter and ZonuDexLibrary, removed the deadline argument
    function _swapTokensForETH(uint amountIn, address fromToken)
        internal
    {
        IZonuDexPair pairToUse = IZonuDexPair(pairFor(fromToken, WETH));
        ...
        require(reserveIn > 0 && reserveOut > 0, 'ZonuDexFeeReceiver: INSUFFICIENT_LIQUIDITY');
    ...
    // Take what was charged as protocol fee from the ZonuDex pair liquidity
    function takeProtocolFee(IZonuDexPair[] calldata pairs) external {
